...
<div style="text-align:inherit !important; font-size:1.1em; font-family: calibri;margin-bottom:10%;">Un réseau électrique est notamment constitué de nœuds (postes de transformation d’électricité, poste de source d'électricité). La vulnérabilité d’un réseau peut difficilement être appréhendée par un acteur autre que le gestionnaire de réseau lui-même, car les travaux de protection ne sont pas toujours visibles, et parce que des substituts peuvent avoir été envisagés pour assurer un fonctionnement alternatif ou dégradé. L’indicateur cherche à identifier les postes électriques susceptibles d’être endommagés par l'inondation.</div>
...
indicateurs <- st_read("layers/indicateurs.geojson", quiet = TRUE) %>%
  st_drop_geometry() %>%
  select(nom, s210a) %>%
   mutate(s210a = replace_na(s210a, 0))

# Sauvegarder au format JSON dans le dossier _site
write_json(indicateurs, "layers/data_s210a.json", auto_unbox = TRUE)
...
map = {
  ...
  maxzoom: 19
    ...
    // Créer le conteneur
        ...
        legend.innerHTML = `
          <strong>Nombre de postes électriques <br> (répartition et distribution) <br> en zone inondable </strong><br>
          <div><span style="background:#d9d9d9;width:12px;height:12px;display:inline-block;margin-right:5px;"></span> 0</div>
          <div><span style="background:#fff5eb;width:12px;height:12px;display:inline-block;margin-right:5px;border:1px solid #ccc;"></span> < 3 </div>
          <div><span style="background:#fd9243;width:12px;height:12px;display:inline-block;margin-right:5px;"></span> 3-11 </div>
          <div><span style="background:#7f2704;width:12px;height:12px;display:inline-block;margin-right:5px;"></span> > 11 </div>`;
        ...
        const visibility = map.getLayoutProperty('indicateurs210aLayer', 'visibility');
        if (visibility === 'none') {
    ...
    // Ajout des couches vectorielles
      layersData.forEach(layer => {
        ...
        if (layer.type === 'fill') {
          if (layer.dynamicColor) {
            ...
            layerConfig.paint['fill-color'] = [
              ...
              ['<', ['get', layer.dynamicColor.property], 3], '#fff5eb',
              ['<', ['get', layer.dynamicColor.property], 11], '#fd9243',
              ['>', ['get', layer.dynamicColor.property], 11], '#7f2704',
              '#d9d9d9' // fallback
            ];
          } else {
        ...
        map.addLayer(layerConfig);
         ...
         map.addLayer({
                  id: 'indicateurs210aLabels',
                  type: 'symbol',
                  source: 'indicateurs210aLayer', // le même id que ta source GeoJSON
                  layout: {
                    'text-field': ['get', 'nom'],
                    'text-size': 11,
        ...
        map.addLayer({
                 id: 'indicateurs210a-hover',
                 type: 'line',
                 source: 'indicateurs210aLayer', // même source
                 paint: {
                   'line-color': '#000',
                   'line-width': 2
                  },
      ...
      map.on('click', 'indicateurs210aLayer', (e) => {
        ...
        const s210aValue = parseFloat(feature.properties.s210a);
        const dommages = (Number(s210aValue) || 0).toLocaleString('fr-FR');
        ...
        <style>
          ...
          <center>${nom}</center>
            ...
            <p style="margin:auto;"><strong>Nombre de postes électriques <br> (répartition et distribution) en zone inondable 
 :</strong> ${dommages}</p>
          </div>
        `;
         ...
         document.getElementById('donnees').innerHTML = `
          <div style="text-align:center; font-family:sans-serif; font-size:16px;">
            ...
            <div style="margin-top:10px;">
              <strong style="font-size:1.1em;"><i class="fas fa-charging-station"></i> Nombre de postes électriques (répartition et distribution) <br> en zone inondable :</strong> ${dommages}
            </div>
          </div>
        `;
      });
      ...
      map.on('mousemove', 'indicateurs210aLayer', (e) => {
        if (e.features.length > 0) {
          const nomCommune = e.features[0].properties.nom;
      
          map.setFilter('indicateurs210a-hover', ['==', 'nom', nomCommune]);
      
          map.getCanvas().style.cursor = 'pointer';
        }
      });

        map.on('mouseleave', 'indicateurs210aLayer', () => {
          map.setFilter('indicateurs210a-hover', ['==', 'nom', '']);
    ...
    const aliasLayerIds = {
      ...
      indicateurs210aLayer: "Nombre de postes électriques (répartition et distribution) en zone inondable",
...
indicateur_s210a <- st_read("layers/indicateurs.geojson", quiet = TRUE) %>%
  select(insee_com, nom, population, s210a) %>%
  mutate(
    s210a = replace_na(s210a, 0)
  ) %>%
  st_drop_geometry() %>%
  rename(
    Commune = nom,
    ...
    "Nombre de postes électriques (répartition et distribution) en zone inondable" = s210a
  ) %>%
  mutate(
...
// Charger les données
data = FileAttachment("layers/data_s210a.json").json()

// Label du slider
html`<label for="slider-s210a" style="display: block;
  margin-top: 1em;
  font-weight: bold;
  margin-bottom: 1em;
...
data_s210a = FileAttachment("layers/data_s210a.json").json()

viewof slider_s210a = InputsTaux.range([0, 21], {
  value: 9,
  step: 3//
  //label: "Linéaire d'itinéraires routiers interceptés en km"
})

sorted_s210a = data_s210a
  .map(d => ({ ...d, s210a: +d.s210a || 0 }))
  .filter(d => d.s210a <= slider_s210a)
  .sort((a, b) => a.s210a - b.s210a)

PlotTaux.plot({
  marks: [
    PlotTaux.barY(sorted_s210a, {
      x: d => d.nom,
      y: d => d.s210a,
        fill: d => {
        const val = d.s210a;
        if (val === 0) return "#d9d9d9";
        if (val < 3) return "#fff5eb";
        if (val <=11) return "#fd9243";
        if (val >11) return "#7f2704";
        return "#67000d"; // >4
        
      }
    }),
    PlotTaux.text(sorted_s210a, {
      x: d => d.nom,
      y: d => d.s210a,
      text: d => d.s210a.toLocaleString("fr-FR"),
      dy: -8,
      fontSize: 9,
      fill: "black",
      textAnchor: "middle"
  ...
  y: {
    grid: true,
    label: "Nombre de postes électriques (répartition et distribution) en zone inondable",
    domain: [0, Math.max(...sorted_s210a.map(d => d.s210a))],
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    ...
    <th style="border:1px solid;">Ind2/10a</th>
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">Nombre de postes électriques (répartition et distribution) en zone inondable</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">Axe 2/4 Les dommages aux infrastructures et aux réseaux</td>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#BEDBE8; border:1px solid; font-weight: bold;">Source de vulnérabilité</td>
    <td colspan="3" style="border:1px solid;">S2/10 Érosion, destructions, dépôts sur les infrastructures, espaces, ouvrages et réseaux (érosion,
déchets…)</td>
  </tr>
</table>
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">Les éléments pris en compte pour cet indicateur sont les postes de distribution et sources du
réseau électrique qui intersectent la couche « zone inondable ».
Les éléments seront agrégés par commune.</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">p_obj2_cout_dommages.s210a_postes_elec_zi</td>
  </tr>
  <tr>
  <tr>
    <td colspan="2" style="background-color:#BEDBE8; border:1px solid; font-weight: bold;">Variables mobilisées</td>
    <td colspan="3" style="border:1px solid;">Zx : hauteur d’eau > 0, <br>Res2 : électricité</td>
  </tr>
  <tr>
  <tr>
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    <td colspan="2" style="border:1px solid;">Postes distribution</td>
    <td style="border:1px solid;">2021</td>
    <td style="border:1px solid;">Nationale</td>
    <td style="border:1px solid;">Shape</td>
    <td style="border:1px solid;">Point</td>
    <td colspan="2" style="border:1px solid;">ENEDIS</td>
    <td colspan="2" style="border:1px solid;">poste_electrique_d86_2021.shp</td>
  </tr>
  <tr>
    <td colspan="2" style="border:1px solid;">Postes distribution</td>
    <td style="border:1px solid;">2021</td>
    ...
    <td style="border:1px solid;">Shape</td>
    <td style="border:1px solid;">Point</td>
    <td colspan="2" style="border:1px solid;">SRD</td>
    <td colspan="2" style="border:1px solid;">postes_de_distribution_publique_postes_htabt.shp</td>
  </tr>
  <tr>
    <td colspan="2" style="border:1px solid;">Postes sources</td>
    <td style="border:1px solid;">2021</td>
    <td style="border:1px solid;">Nationale</td>
    <td style="border:1px solid;">Shape</td>
    <td style="border:1px solid;">Point</td>
    <td colspan="2" style="border:1px solid;">ENEDIS</td>
    <td colspan="2" style="border:1px solid;">poste_source_d86_2021.shp</td>
  </tr>
  <tr>
    <td colspan="2" style="border:1px solid;">Postes sources</td>
    <td style="border:1px solid;">2021</td>
    ...
    <td style="border:1px solid;">Point</td>
    <td colspan="2" style="border:1px solid;">SRD</td>
    <td colspan="2" style="border:1px solid;">postes_sources_htbhta_et_postes_de_repartition_htahta.shp</td>
  </tr>
  <tr>
