...
<div style="text-align:inherit !important; font-size:1.1em; font-family: calibri;margin-bottom:10%;"> Selon les caractéristiques de l’inondation, la saisonnalité de ces inondations et les types de cultures, il est possible d’évaluer le montant des pertes à attendre.</div>
...
#communes_images <- readxl::read_xlsx("layers/communes_images.xlsx", quiet = TRUE)

# Charger le GeoJSON
indicateurs <- st_read("layers/indicateurs.geojson", quiet = TRUE) %>%
  st_drop_geometry() %>%
  select(nom, s26a) %>%
   mutate(s26a = replace_na(s26a, 0))

# Sauvegarder au format JSON dans le dossier _site
write_json(indicateurs, "layers/data_s26a.json", auto_unbox = TRUE)
...
map = {
  ...
  maxzoom: 19
    ...
    // Créer le conteneur
        ...
        legend.innerHTML = `
          <strong>Dommages aux cultures en €</strong><br>
          <div><span style="background:#d9d9d9;width:12px;height:12px;display:inline-block;margin-right:5px;"></span> 0</div>
          <div><span style="background:#f7fcf5;width:12px;height:12px;display:inline-block;margin-right:5px;border:1px solid #ccc;"></span> < 5000 </div>
          <div><span style="background:#b2e0ab;width:12px;height:12px;display:inline-block;margin-right:5px;border:1px solid #ccc;"></span> 5000 - 24000 </div>
          <div><span style="background:#3da75a;width:12px;height:12px;display:inline-block;margin-right:5px;"></span>24000 - 60000</div>
          <div><span style="background:#00441b;width:12px;height:12px;display:inline-block;margin-right:5px;"></span> > 60000</div>
        `;
        ...
        const visibility = map.getLayoutProperty('indicateurs26aLayer', 'visibility');
        if (visibility === 'none') {
    ...
    // Ajout des couches vectorielles
      layersData.forEach(layer => {
        ...
        if (layer.type === 'fill') {
          if (layer.dynamicColor) {
            ...
            layerConfig.paint['fill-color'] = [
              ...
              ['<', ['get', layer.dynamicColor.property], 5000], '#f7fcf5',
              ['<', ['get', layer.dynamicColor.property], 24000], '#b2e0ab',
              ['<', ['get', layer.dynamicColor.property], 60000], '#3da75a',
              ['>', ['get', layer.dynamicColor.property], 60000], '#00441b',
              '#d9d9d9' // fallback
            ];
          } else {
        ...
        map.addLayer(layerConfig);
         ...
         map.addLayer({
                  id: 'indicateurs26aLabels',
                  type: 'symbol',
                  source: 'indicateurs26aLayer', // le même id que ta source GeoJSON
                  layout: {
                    'text-field': ['get', 'nom'],
                    'text-size': 11,
        ...
        map.addLayer({
                 id: 'indicateurs26a-hover',
                 type: 'line',
                 source: 'indicateurs26aLayer', // même source
                 paint: {
                   'line-color': '#000',
                   'line-width': 2
                  },
      ...
      map.on('click', 'indicateurs26aLayer', (e) => {
        ...
        const s26aValue = parseFloat(feature.properties.s26a);
        const dommages = (Number(s26aValue) || 0).toLocaleString('fr-FR');
        ...
        <style>
          ...
          <center>${nom}</center>
            ...
            <p style="margin:auto;"><strong>Dommages aux cultures en € :</strong> ${dommages}</p>
          </div>
        `;
        ...
        new maplibregl.Popup({closeButton: false})
         ...
         <strong style="font-size:1.1em"><i class="fas fa-seedling"></i> Dommages aux cultures en € :</strong> ${dommages}<br>
         `;
      });



      map.on('mousemove', 'indicateurs26aLayer', (e) => {
        if (e.features.length > 0) {
          const nomCommune = e.features[0].properties.nom;
      
          map.setFilter('indicateurs26a-hover', ['==', 'nom', nomCommune]);
      
          map.getCanvas().style.cursor = 'pointer';
        }
      });

        map.on('mouseleave', 'indicateurs26aLayer', () => {
          map.setFilter('indicateurs26a-hover', ['==', 'nom', '']);
    ...
    const aliasLayerIds = {
      ...
      indicateurs26aLayer: "Dommages aux cultures en €",
...
indicateur_s26a <- st_read("layers/indicateurs.geojson", quiet = TRUE) %>%
  select(insee_com, nom, population, s26a) %>%
  mutate(
    s26a = replace_na(s26a, 0)
  ) %>%
  st_drop_geometry() %>%
  rename(
    Commune = nom,
    ...
    `Dommages aux cultures en €` = s26a
  ) %>%
  mutate(
    Population = format(Population, big.mark = " ", scientific = FALSE),
    `Dommages aux cultures en €` = format(`Dommages aux cultures en €`, big.mark = " ", scientific = FALSE)
  )

# Affichage avec reactable
reactable(indicateur_s26a,
          filterable = TRUE,
          striped = TRUE,
          sortable = TRUE,
          resizable = TRUE,
...
// Charger les données
data = FileAttachment("layers/data_s26a.json").json()

// Label du slider
html`<label for="slider-s114c" style="display: block;
  margin-top: 1em;
  font-weight: bold;
  margin-bottom: 1em;
  ...
  margin-right: 20%;
  margin-left: 20%;">
  Dommages aux cultures en €
</label>`

// Slider


data_s26a = FileAttachment("layers/data_s26a.json").json()

viewof slider_s26a = InputsTaux.range([0, 120000], {
  value: 60000,
  step: 10000,
  //label: "Dommages aux logements (M€)"
})

sorted_s26a = data_s26a
  .map(d => ({ ...d, s26a: +d.s26a || 0 }))
  .filter(d => d.s26a <= slider_s26a)
  .sort((a, b) => a.s26a - b.s26a)

PlotTaux.plot({
  marks: [
    PlotTaux.barY(sorted_s26a, {
      x: d => d.nom,
      y: d => d.s26a,
        fill: d => {
        const val = d.s26a;
        if (val === 0) return "#d9d9d9";
        if (val < 5000) return "#f7fcf5";
        if (val < 24000) return "#b2e0ab";
        if (val < 60000) return "#3da75a";
        if (val > 60000) return "#00441b";
        return "#67000d"; // >4
      }
    }),
    PlotTaux.text(sorted_s26a, {
      x: d => d.nom,
      y: d => d.s26a,
      text: d => d.s26a.toLocaleString("fr-FR"),
      dy: -8,
      fontSize: 9,
      fill: "black",
      textAnchor: "middle"
  ...
  y: {
    grid: true,
    label: "Dommages aux cultures en €",
    domain: [0, Math.max(...sorted_s26a.map(d => d.s26a))],
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    <th style="background-color:#88A9E3; border:1px solid;" colspan="2">Numéro d'indicateur</th>
    <th style="border:1px solid;">Ind2/6a</th>
    <th style="background-color:#88A9E3; border:1px solid;">Date de mise à jour</th>
    <th style="border:1px solid;">31/05/2025</th>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#88A9E3; border:1px solid; font-weight: bold;">Intitulé</td>
    <td colspan="3" style="border:1px solid;">Dommages aux cultures en fonction de paramètres hydrauliques</td>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#88A9E3; border:1px solid; font-weight: bold;">Objectif de la SNGRI</td>
  ...
  <tr>
    <td colspan="2" style="background-color:#88A9E3; border:1px solid; font-weight: bold;">Axe de vulnérabilité</td>
    <td colspan="3" style="border:1px solid;">Axe 2/3 Les dommages aux activités et aux biens</td>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#88A9E3; border:1px solid; font-weight: bold;">Source de vulnérabilité</td>
    <td colspan="3" style="border:1px solid;">S2/6 Inondation, érosion, dépôts au sein des exploitations agricoles</td>
  </tr>
</table>
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    <td colspan="2" style="background-color:#88A9E3; border:1px solid; font-weight: bold;">Modalités du calcul</td>
    <td colspan="3" style="border:1px solid;">Application de la méthode du référentiel national.
On utilise le tableau du guide AMC sur les dommages aux cultures (pour l’aléa fluvial), qui
dépendent des saisons, des types de culture, de la durée et de la hauteur de submersion.
La couche des cultures RPG de l’IGN (millésime 2019) est croisée avec la zone inondable
contenant l’information de hauteur d’eau. Afin de prendre en compte toutes les saisons, un ratio
est appliqué : 40 % des dommages pour le printemps et l’automne et 10 % pour l’été et l’hiver,
l’ensemble faisant 100 %.</td>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#88A9E3; border:1px solid; font-weight: bold;">Échelle de représentation</td>
    <td colspan="3" style="border:1px solid;">Communale</td>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#88A9E3; border:1px solid; font-weight: bold;">Points de vigilance</td>
    <td colspan="3" style="border:1px solid;">Tous les types de cultures du RPG ne sont pas répertoriés dans le guide AMC. Pour les cultures absentes, une correspondance avec celles existantes a été réalisée.</td>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#88A9E3; border:1px solid; font-weight: bold;">Table cible</td>
    <td colspan="3" style="border:1px solid;">p_obj2_cout_dommages.s26a_dommages_cultures_zi</td>
  </tr>
  <tr>
  <tr>
    <td colspan="2" style="background-color:#88A9E3; border:1px solid; font-weight: bold;">Variables mobilisées</td>
    <td colspan="3" style="border:1px solid;">Zx : zone inondable,<br> Zf : durée de submersion, <br> Oc7 : cultures</td>
  </tr>
  <tr>
  <tr>
    <td colspan="2" style="background-color:#88A9E3; border:1px solid; font-weight: bold;">Indicateur commun SLGRI Vienne/Clain</td>
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    <td colspan="10" style="background-color:#88A9E3; border:1px solid; font-weight: bold;">Données sources</td>
  </tr>
  <tr>
    <th style="background-color:#88A9E3; border:1px solid;" colspan="2">Désignation</th>
    <th style="background-color:#88A9E3; border:1px solid;">Millésime</th>
    <th style="background-color:#88A9E3; border:1px solid;">Nationale/locale</th>
    <th style="background-color:#88A9E3; border:1px solid;">Format</th>
    <th style="background-color:#88A9E3; border:1px solid;">Type</th>
    <th style="background-color:#88A9E3; border:1px solid;" colspan="2">Producteur</th>
    <th style="background-color:#88A9E3; border:1px solid;" colspan="2">Nom fichier</th>
  </tr>
  <tr>
  ...
  <tr>
    <td colspan="2" style="border:1px solid;">Parcelles agricoles Nouvelle-Aquitaine</td>
    <td style="border:1px solid;">2019</td>
    <td style="border:1px solid;">Nationale</td>
    <td style="border:1px solid;">Shape</td>
    <td style="border:1px solid;">Polygone</td>
    <td colspan="2" style="border:1px solid;">IGN</td>
    <td colspan="2" style="border:1px solid;">parcelles_graphiques.shp</td>
  </tr>
  <tr>
    <td colspan="2" style="border:1px solid;">Fonction de dommages relative aux logements (pour l’aléa fluvial) du
guide AMC</td>
    ...
    <td colspan="2" style="border:1px solid;">amc_dommages_fluvial_cultures.csv</td>
  </tr>
  <tr>
