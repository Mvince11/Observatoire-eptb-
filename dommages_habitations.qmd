...
<div style="text-align:inherit !important; font-size:1.1em; font-family: calibri;margin-bottom:10%;"> La pénétration d’eau dans les logements entraîne des dégâts dépendant principalement de la hauteur et de la durée de submersion. Le présent indicateur permet d’évaluer les dégâts aux habitations pour l’inondation retenue.</div>
...
indicateurs <- st_read("layers/indicateurs.geojson", quiet = TRUE) %>%
  st_drop_geometry() %>%
  select(nom, s22a) %>%
   mutate(s22a = replace_na(s22a, 0))

# Sauvegarder au format JSON dans le dossier _site
write_json(indicateurs, "layers/data_s22a.json", auto_unbox = TRUE)
...
map = {
  ...
  maxzoom: 19
    ...
    // Créer le conteneur
        ...
        legend.innerHTML = `
          <strong>Dommages aux logements (M€)</strong><br>
          ...
          <div><span style="background:#ffaaaa;width:12px;height:12px;display:inline-block;margin-right:5px;border:1px solid #ccc;"></span> 1 - 5 </div>
          <div><span style="background:#ff5555;width:12px;height:12px;display:inline-block;margin-right:5px;"></span> 5 - 10</div>
          <div><span style="background:#ff0000;width:12px;height:12px;display:inline-block;margin-right:5px;"></span> > 10 </div>
        `;
        ...
        const visibility = map.getLayoutProperty('indicateurs22aLayer', 'visibility');
        if (visibility === 'none') {
    ...
    // Ajout des couches vectorielles
      layersData.forEach(layer => {
        ...
        if (layer.type === 'fill') {
          if (layer.dynamicColor) {
            ...
            layerConfig.paint['fill-color'] = [
              ...
              ['<', ['get', layer.dynamicColor.property], 1000000], '#ffffff',
              ['<', ['get', layer.dynamicColor.property], 5000000], '#ffaaaa',
              ['<', ['get', layer.dynamicColor.property], 10000000], '#ff5555',
              ['>', ['get', layer.dynamicColor.property], 10000000], '#ff0000',
              '#d9d9d9' // fallback
            ];
          } else {
        ...
        map.addLayer(layerConfig);
         ...
         map.addLayer({
                  id: 'indicateurs22aLabels',
                  type: 'symbol',
                  source: 'indicateurs22aLayer', // le même id que ta source GeoJSON
                  layout: {
                    'text-field': ['get', 'nom'],
                    'text-size': 11,
        ...
        map.addLayer({
                 id: 'indicateurs22a-hover',
                 type: 'line',
                 source: 'indicateurs22aLayer', // même source
                 paint: {
                   'line-color': '#000',
                   'line-width': 2
                  },
      ...
      map.on('click', 'indicateurs22aLayer', (e) => {
        ...
        const s22aValue = parseFloat(feature.properties.s22a);
        const dommages = (Number(s22aValue) || 0).toLocaleString('fr-FR');
        ...
        <style>
          ...
          <center>${nom}</center>
            ...
            <p style="margin:auto;"><strong>Dommages aux logements (M€) :</strong> ${dommages}</p>
          </div>
        `;
        ...
        new maplibregl.Popup({closeButton: false})
         ...
         <strong style="font-size:1.1em"><i class="fas fa-euro-sign"></i> Dommages aux logements (M€) :</strong> ${dommages}<br>
         `;
      });



      map.on('mousemove', 'indicateurs22aLayer', (e) => {
        if (e.features.length > 0) {
          const nomCommune = e.features[0].properties.nom;
      
          map.setFilter('indicateurs22a-hover', ['==', 'nom', nomCommune]);
      
          map.getCanvas().style.cursor = 'pointer';
        }
      });

        map.on('mouseleave', 'indicateurs22aLayer', () => {
          map.setFilter('indicateurs22a-hover', ['==', 'nom', '']);
    ...
    const aliasLayerIds = {
      ...
      indicateurs22aLayer: "Dommages aux logements (M€)",
...
indicateur_s22a <- st_read("layers/indicateurs.geojson", quiet = TRUE) %>%
  select(insee_com, nom, population, s22a) %>%
  mutate(
    s22a = replace_na(s22a, 0)
  ) %>%
  st_drop_geometry() %>%
  rename(
    Commune = nom,
    ...
    `Dommages aux logements (M€)` = s22a
  ) %>%
  mutate(
    Population = format(Population, big.mark = " ", scientific = FALSE),
    `Dommages aux logements (M€)` = format(`Dommages aux logements (M€)`, big.mark = " ", scientific = FALSE)
  )

# Affichage avec reactable
reactable(indicateur_s22a,
          filterable = TRUE,
          striped = TRUE,
          sortable = TRUE,
          resizable = TRUE,
...
// Charger les données
data = FileAttachment("layers/data_s22a.json").json()
...
html`<label for="slider-s114c" style="display: block;
  ...
  margin-right: 20%;
  margin-left: 20%;">
  Dommages aux logements (M€)
</label>`

// Slider


data_s22a = FileAttachment("layers/data_s22a.json").json()

viewof slider_s22a = InputsTaux.range([0, 65000000], {
  value: 10000000,
  step: 1000000,
  label: "Dommages aux logements (M€)"
})

sorted_s22a = data_s22a
  .map(d => ({ ...d, s22a: +d.s22a || 0 }))
  .filter(d => d.s22a <= slider_s22a)
  .sort((a, b) => a.s22a - b.s22a)

PlotTaux.plot({
  marks: [
    PlotTaux.barY(sorted_s22a, {
      x: d => d.nom,
      y: d => d.s22a,
        fill: d => {
        const val = d.s22a;
        if (val === 0) return "#d9d9d9";
        if (val < 1000000) return "#ffffff";
        if (val < 5000000) return "#ffaaaa";
        if (val < 10000000) return "#ff5555";
        if (val > 10000000) return "#ff0000";
        return "#67000d"; // >4
      }
    }),
    PlotTaux.text(sorted_s22a, {
      x: d => d.nom,
      y: d => d.s22a,
      text: d => d.s22a.toLocaleString("fr-FR"),
      dy: -8,
      fontSize: 9,
      fill: "black",
      textAnchor: "middle"
  ...
  y: {
    grid: true,
    label: "Nombre",
    domain: [0, Math.max(...sorted_s22a.map(d => d.s22a))],
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    ...
    <th style="border:1px solid;">Ind2/2a</th>
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">Dommages aux bâtiments d’habitation en fonction de la hauteur d’eau et de la durée de submersion</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">Axe 2/1 Les dommages aux bâtiments</td>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#88A9E3; border:1px solid; font-weight: bold;">Source de vulnérabilité</td>
    <td colspan="3" style="border:1px solid;">S2/2 Pénétration d’eau dans les logements</td>
  </tr>
</table>
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">Application de la méthode du référentiel national. Les dommages aux bâtiments d’habitation sont estimés en fonction de la hauteur d’eau et de la durée de submersion d’après le guide AMC. La durée de submersion est supérieure à 48 h sur tout le territoire. Les résultats sont ensuite agrégés par commune.</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">Pas de vigilance particulière.</td>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#88A9E3; border:1px solid; font-weight: bold;">Table cible</td>
    <td colspan="3" style="border:1px solid;">p_obj2_cout_dommages.s22a_dommages_logt</td>
  </tr>
  <tr>
  <tr>
    <td colspan="2" style="background-color:#88A9E3; border:1px solid; font-weight: bold;">Variables mobilisées</td>
    <td colspan="3" style="border:1px solid;">Zx : zone inondable,<br> Zf : durée de submersion, <br> Oc2 : logements</td>
  </tr>
  <tr>
  <tr>
...
guide AMC</td>
    ...
    <td colspan="2" style="border:1px solid;">CGDD (MTE)</td>
    <td colspan="2" style="border:1px solid;">amc_dommages_fluvial_logements.csv</td>
  </tr>
  <tr>
