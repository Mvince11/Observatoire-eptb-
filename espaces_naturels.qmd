...
<div style="text-align:inherit !important; font-size:1.1em; font-family: calibri;margin-bottom:10%;"> Les protections des espaces naturels peuvent être justifiées par la sensibilité de ces espaces. Il s’agit de les identifier pour les mettre en perspective avec les pollutions potentielles.</div>
...
indicateurs <- st_read("layers/indicateurs.geojson", quiet = TRUE) %>%
  st_drop_geometry() %>%
  select(nom, s25c) %>%
   mutate(s25c = replace_na(s25c, 0))

# Sauvegarder au format JSON dans le dossier _site
write_json(indicateurs, "layers/data_s25c.json", auto_unbox = TRUE)
...
map = {
  ...
  maxzoom: 19
    ...
    // Créer le conteneur
        ...
        legend.innerHTML = `
          <strong>Epaces naturels protégés en ZI (ha)</strong><br>
          <div><span style="background:#d9d9d9;width:12px;height:12px;display:inline-block;margin-right:5px;"></span> 0</div>
          <div><span style="background:#f7fcf5;width:12px;height:12px;display:inline-block;margin-right:5px;border:1px solid #ccc;"></span> < 2 </div>
          <div><span style="background:#7bc87c;width:12px;height:12px;display:inline-block;margin-right:5px;border:1px solid #ccc;"></span> ≥ 2 </div>
        `;
        ...
        const visibility = map.getLayoutProperty('indicateurs25cLayer', 'visibility');
        if (visibility === 'none') {
    ...
    // Ajout des couches vectorielles
      layersData.forEach(layer => {
        ...
        if (layer.type === 'fill') {
          if (layer.dynamicColor) {
            ...
            layerConfig.paint['fill-color'] = [
              ...
              ['<', ['get', layer.dynamicColor.property], 2], '#f7fcf5',
              ['>', ['get', layer.dynamicColor.property], 2], '#7bc87c',
              '#d9d9d9' // fallback
            ];
          } else {
        ...
        map.addLayer(layerConfig);
         ...
         map.addLayer({
                  id: 'indicateurs25cLabels',
                  type: 'symbol',
                  source: 'indicateurs25cLayer', // le même id que ta source GeoJSON
                  layout: {
                    'text-field': ['get', 'nom'],
                    'text-size': 11,
        ...
        map.addLayer({
                 id: 'indicateurs25c-hover',
                 type: 'line',
                 source: 'indicateurs25cLayer', // même source
                 paint: {
                   'line-color': '#000',
                   'line-width': 2
                  },
      ...
      map.on('click', 'indicateurs25cLayer', (e) => {
        ...
        const s25cValue = parseFloat(feature.properties.s25c);
        const dommages = (Number(s25cValue) || 0).toLocaleString('fr-FR') + ' ha';
        ...
        <style>
          ...
          <center>${nom}</center>
            ...
            <p style="margin:auto;"><strong>Epaces naturels protégés en ZI :</strong> ${dommages}</p>
          </div>
        `;
        ...
        new maplibregl.Popup({closeButton: false})
         ...
         <strong style="font-size:1.1em"><i class="fas fa-tree"></i> Epaces naturels protégés en ZI :</strong> ${dommages}<br>
         `;
      });



      map.on('mousemove', 'indicateurs25cLayer', (e) => {
        if (e.features.length > 0) {
          const nomCommune = e.features[0].properties.nom;
      
          map.setFilter('indicateurs25c-hover', ['==', 'nom', nomCommune]);
      
          map.getCanvas().style.cursor = 'pointer';
        }
      });

        map.on('mouseleave', 'indicateurs25cLayer', () => {
          map.setFilter('indicateurs25c-hover', ['==', 'nom', '']);
    ...
    const aliasLayerIds = {
      ...
      indicateurs25cLayer: "Epaces naturels protégés en ZI (ha)",
...
indicateur_s25c <- st_read("layers/indicateurs.geojson", quiet = TRUE) %>%
  select(insee_com, nom, population, s25c) %>%
  mutate(
    s25c = replace_na(s25c, 0)
  ) %>%
  st_drop_geometry() %>%
  rename(
    Commune = nom,
    ...
    `Epaces naturels protégés en ZI (ha)` = s25c
  ) %>%
  mutate(
...
// Charger les données
data = FileAttachment("layers/data_s25c.json").json()
...
html`<label for="slider-s114c" style="display: block;
  ...
  margin-right: 20%;
  margin-left: 20%;">
  Epaces naturels protégés en ZI (ha)
</label>`

// Slider


data_s25c = FileAttachment("layers/data_s25c.json").json()

viewof slider_s25c = InputsTaux.range([0, 5], {
  value: 2,
  step: 1//,
  //label: "Dommages aux logements (M€)"
})

sorted_s25c = data_s25c
  .map(d => ({ ...d, s25c: +d.s25c || 0 }))
  .filter(d => d.s25c <= slider_s25c)
  .sort((a, b) => a.s25c - b.s25c)

PlotTaux.plot({
  marks: [
    PlotTaux.barY(sorted_s25c, {
      x: d => d.nom,
      y: d => d.s25c,
        fill: d => {
        const val = d.s25c;
        if (val === 0) return "#d9d9d9";
        if (val < 2) return "#f7fcf5";
        if (val > 2) return "#7bc87c";
        return "#7bc87c"; // >4
      }
    }),
    PlotTaux.text(sorted_s25c, {
      x: d => d.nom,
      y: d => d.s25c,
      text: d => d.s25c.toLocaleString("fr-FR"),
      dy: -8,
      fontSize: 9,
      fill: "black",
      textAnchor: "middle"
  ...
  y: {
    grid: true,
    label: "Epaces naturels protégés en ZI (ha)",
    domain: [0, Math.max(...sorted_s25c.map(d => d.s25c))],
    //ticks: 3,
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    ...
    <th style="border:1px solid;">Ind2/5c</th>
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">Surface d’espaces naturels protégés vulnérables</td>
  </tr>
  <tr>
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">On croise les différentes couches de zonages d’espace naturel protégé avec la zone inondable. Il s’agit des couches ENP APB (aire de protection de biotope), ENP RNN (réserve naturelle
nationale), N2000 ZPS (zone de protection spéciale) et N2000 ZSC (zone spéciale de conservation).</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">p_obj2_cout_dommages.s25c_nat_prot_zi</td>
  </tr>
  <tr>
  <tr>
    <td colspan="2" style="background-color:#88A9E3; border:1px solid; font-weight: bold;">Variables mobilisées</td>
    <td colspan="3" style="border:1px solid;">Zx : zone inondable,<br> Oc8 : espaces naturels protégés</td>
  </tr>
  <tr>
  <tr>
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    <td colspan="2" style="border:1px solid;">N2000 ZPS (espèces)</td>
    <td style="border:1px solid;">2020</td>
    <td style="border:1px solid;">Locale</td>
    <td style="border:1px solid;">Shape</td>
    <td style="border:1px solid;">Polygone</td>
    <td colspan="2" style="border:1px solid;">DREAL NA</td>
    <td colspan="2" style="border:1px solid;">N_N2000_ZPS_ZINF_S_dept86.shp</td>
  </tr>
  <tr>
    <td colspan="2" style="border:1px solid;">N2000 ZSC (habitat)</td>
    <td style="border:1px solid;">2020</td>
    <td style="border:1px solid;">Nationale</td>
    <td style="border:1px solid;">Shape</td>
    <td style="border:1px solid;">Polygone</td>
    <td colspan="2" style="border:1px solid;">DREAL NA</td>
    <td colspan="2" style="border:1px solid;">N_N2000_ZSC_ZINF_S_dept86.shp</td>
  </tr>
  <tr>
    <td colspan="2" style="border:1px solid;">Arrêté de Protection Biotope</td>
    <td style="border:1px solid;">2020</td>
    <td style="border:1px solid;">Nationale</td>
    <td style="border:1px solid;">Shape</td>
    <td style="border:1px solid;">Polygone</td>
    <td colspan="2" style="border:1px solid;">DREAL NA</td>
    <td colspan="2" style="border:1px solid;">N_ENP_APB_S_dept86.shp</td>
  </tr>
  <tr>
    <td colspan="2" style="border:1px solid;">Réserves naturelles nationales</td>
    <td style="border:1px solid;">2020</td>
    <td style="border:1px solid;">Nationale</td>
    <td style="border:1px solid;">Shape</td>
    <td style="border:1px solid;">Polygone</td>
    <td colspan="2" style="border:1px solid;">DREAL NA</td>
    <td colspan="2" style="border:1px solid;">N_ENP_RNN_S_dept86.shp</td>
  </tr>
  <tr>
