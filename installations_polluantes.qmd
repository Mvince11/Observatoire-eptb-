...
<div style="text-align:inherit !important; font-size:1.1em; font-family: calibri;margin-bottom:10%;"> L’inondation d’installations polluantes est potentiellement la source de pollution. Il s’agit d’identifier les principaux établissements polluants susceptibles d’être concernés. Seule une étude de danger poussée peut permettre d’aller plus loin.</div>
...
indicateurs <- st_read("layers/indicateurs.geojson", quiet = TRUE) %>%
  st_drop_geometry() %>%
  select(nom, s25b) %>%
   mutate(s25b = replace_na(s25b, 0))

# Sauvegarder au format JSON dans le dossier _site
write_json(indicateurs, "layers/data_s25b.json", auto_unbox = TRUE)
...
map = {
  ...
  maxzoom: 19
    ...
    // Créer le conteneur
        ...
        legend.innerHTML = `
          <strong>Nombre d’installations <br> susceptibles de déverser <br> des produits polluants</strong><br>
          <div><span style="background:#d9d9d9;width:12px;height:12px;display:inline-block;margin-right:5px;"></span> 0</div>
          <div><span style="background:#ffffff;width:12px;height:12px;display:inline-block;margin-right:5px;border:1px solid #ccc;"></span> 1-2 </div>
          <div><span style="background:#ff7676;width:12px;height:12px;display:inline-block;margin-right:5px;"></span> 2-3 </div>`;
        ...
        const visibility = map.getLayoutProperty('indicateurs25bLayer', 'visibility');
        if (visibility === 'none') {
    ...
    // Ajout des couches vectorielles
      layersData.forEach(layer => {
        ...
        if (layer.type === 'fill') {
          if (layer.dynamicColor) {
            ...
            layerConfig.paint['fill-color'] = [
              ...
              ['<', ['get', layer.dynamicColor.property], 2], '#ffffff',
              ['>=', ['get', layer.dynamicColor.property], 2], '#ff7676',
              '#d9d9d9' // fallback
            ];
          } else {
        ...
        map.addLayer(layerConfig);
         ...
         map.addLayer({
                  id: 'indicateurs25bLabels',
                  type: 'symbol',
                  source: 'indicateurs25bLayer', // le même id que ta source GeoJSON
                  layout: {
                    'text-field': ['get', 'nom'],
                    'text-size': 11,
        ...
        map.addLayer({
                 id: 'indicateurs25b-hover',
                 type: 'line',
                 source: 'indicateurs25bLayer', // même source
                 paint: {
                   'line-color': '#000',
                   'line-width': 2
                  },
      ...
      map.on('click', 'indicateurs25bLayer', (e) => {
        ...
        const s25bValue = parseFloat(feature.properties.s25b);
        const dommages = (Number(s25bValue) || 0).toLocaleString('fr-FR');
        ...
        <style>
          ...
          <center>${nom}</center>
            ...
            <p style="margin:auto;"><strong>Nombre d’installations susceptibles <br> de déverser des produits polluants
 :</strong> ${dommages}</p>
          </div>
        `;
         ...
         document.getElementById('donnees').innerHTML = `
          <div style="text-align:center; font-family:sans-serif; font-size:16px;">
            ...
            <div style="margin-top:10px;">
              <strong style="font-size:1.1em;"><i class="fas fa-oil-can"></i> Nombre d’installations susceptibles de déverser des produits polluants :</strong> ${dommages}
            </div>
          </div>
        `;
      });
      ...
      map.on('mousemove', 'indicateurs25bLayer', (e) => {
        if (e.features.length > 0) {
          const nomCommune = e.features[0].properties.nom;
      
          map.setFilter('indicateurs25b-hover', ['==', 'nom', nomCommune]);
      
          map.getCanvas().style.cursor = 'pointer';
        }
      });

        map.on('mouseleave', 'indicateurs25bLayer', () => {
          map.setFilter('indicateurs25b-hover', ['==', 'nom', '']);
    ...
    const aliasLayerIds = {
      ...
      indicateurs25bLayer: "Nombre d’installations susceptibles de déverser des produits polluants",
...
indicateur_s25b <- st_read("layers/indicateurs.geojson", quiet = TRUE) %>%
  select(insee_com, nom, population, s25b) %>%
  mutate(
    s25b = replace_na(s25b, 0)
  ) %>%
  st_drop_geometry() %>%
  rename(
    Commune = nom,
    ...
    "Nombre d’installations susceptibles de déverser des produits polluants" = s25b
  ) %>%
  mutate(
...
// Charger les données
data = FileAttachment("layers/data_s25b.json").json()

// Label du slider
html`<label for="slider-s25b" style="display: block;
  margin-top: 1em;
  font-weight: bold;
  margin-bottom: 1em;
...
data_s25b = FileAttachment("layers/data_s25b.json").json()

viewof slider_s25b = InputsTaux.range([0, 3], {
  value: 2,
  step: 1//
  //label: "Linéaire d'itinéraires routiers interceptés en km"
})

sorted_s25b = data_s25b
  .map(d => ({ ...d, s25b: +d.s25b || 0 }))
  .filter(d => d.s25b <= slider_s25b)
  .sort((a, b) => a.s25b - b.s25b)

PlotTaux.plot({
  marks: [
    PlotTaux.barY(sorted_s25b, {
      x: d => d.nom,
      y: d => d.s25b,
        fill: d => {
        const val = d.s25b;
        if (val === 0) return "#d9d9d9";
        if (val < 2) return "#F5E9E4";
        if (val >= 2) return "#ff7676";
        return "#67000d"; // >4
        
      }
    }),
    PlotTaux.text(sorted_s25b, {
      x: d => d.nom,
      y: d => d.s25b,
      text: d => d.s25b.toLocaleString("fr-FR"),
      dy: -8,
      fontSize: 9,
      fill: "black",
      textAnchor: "middle"
  ...
  y: {
    grid: true,
    label: "Nombre d’installations susceptibles de déverser des produits polluants",
    domain: [0, Math.max(...sorted_s25b.map(d => d.s25b))],
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    ...
    <th style="border:1px solid;">Ind2/5b</th>
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">Nombre d’installations susceptibles de déverser des produits polluants</td>
  </tr>
  <tr>
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">Les bases de données des installations classées (ICPE), des sites industriels et activités de service (BASIAS) et des stations de traitement des eaux usées ont été croisées avec la zone inondable pour retenir les installations qui sont potentiellement impactées. Un buffer de 10 mètres autour
des points des sites est effectué pour simuler l’emprise des bâtiments. On ne conserve que les
établissements autres que « Activité terminée » et de type « dépôt ou stockage de gaz… »
(V89.07Z, V89.03Z) ou « stockage de produits chimiques » (V89.01Z) de la base BASIAS.
Le décompte des installations est agrégé à la commune.</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">Des données ICPE peuvent être redondantes dans la base BASIAS. Un contrôle visuel des installations est nécessaire sur celles qui se situent dans la zone inondable. Sur notre territoire, ce contrôle n’a relevé aucun doublon.</td>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#BEDBE8; border:1px solid; font-weight: bold;">Table cible</td>
    <td colspan="3" style="border:1px solid;">p_obj2_cout_dommages.s25b_polluant_zi</td>
  </tr>
  <tr>
  <tr>
    <td colspan="2" style="background-color:#BEDBE8; border:1px solid; font-weight: bold;">Variables mobilisées</td>
    <td colspan="3" style="border:1px solid;">Zx : zone inondable, <br> Ag2 : installations polluantes</td>
  </tr>
  <tr>
  <tr>
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    <td colspan="2" style="border:1px solid;">Installations classées</td>
    <td style="border:1px solid;">2021</td>
    <td style="border:1px solid;">Locale</td>
    <td style="border:1px solid;">Shape</td>
    <td style="border:1px solid;">Point</td>
    <td colspan="2" style="border:1px solid;">DREAL NA</td>
    <td colspan="2" style="border:1px solid;">icpe.shp</td>
  </tr>
  <tr>
    <td colspan="2" style="border:1px solid;">BASIAS</td>
    <td style="border:1px solid;">2021</td>
    <td style="border:1px solid;">Nationale</td>
    <td style="border:1px solid;">CSV</td>
    <td style="border:1px solid;">Point</td>
    <td colspan="2" style="border:1px solid;">MTE</td>
    <td colspan="2" style="border:1px solid;">basias_export_d86_sept2021.csv</td>
  </tr>
  <tr>
    <td colspan="2" style="border:1px solid;">BASOL</td>
    <td style="border:1px solid;">2021</td>
    <td style="border:1px solid;">Nationale</td>
    <td style="border:1px solid;">CSV</td>
    <td style="border:1px solid;">Point</td>
    <td colspan="2" style="border:1px solid;">MTE</td>
    <td colspan="2" style="border:1px solid;">basol_result_d86.csv</td>
  </tr>
  <tr>
    <td colspan="2" style="border:1px solid;">Stations d’épuration</td>
    <td style="border:1px solid;">2019</td>
    <td style="border:1px solid;">Nationale</td>
    <td style="border:1px solid;">GPKG</td>
    <td style="border:1px solid;">Point</td>
    <td colspan="2" style="border:1px solid;">MTE</td>
    <td colspan="2" style="border:1px solid;">export_eru_2019.gpkg</td>
  </tr>
  <tr>
