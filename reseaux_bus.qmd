...
<div style="text-align:inherit !important; font-size:1.1em; font-family: calibri;margin-bottom:10%;">L’indicateur cherche à identifier les parties de réseaux de bus situées en zone inondable et qui seraient susceptibles d’être endommagées.</div>
...
indicateurs <- st_read("layers/indicateurs.geojson", quiet = TRUE) %>%
  st_drop_geometry() %>%
  select(nom, s210d) %>%
   mutate(s210d = replace_na(s210d, 0))

# Sauvegarder au format JSON dans le dossier _site
write_json(indicateurs, "layers/data_s210d.json", auto_unbox = TRUE)
...
map = {
  ...
  maxzoom: 19
    ...
    // Créer le conteneur
        ...
        legend.innerHTML = `
          <strong>Linéaire de réseaux de bus urbain <br> et de bus de transport départemental <br> impactés par la zone inondable</strong><br>
          <div><span style="background:#d9d9d9;width:12px;height:12px;display:inline-block;margin-right:5px;"></span> 0</div>
          <div><span style="background:#ffa6a6;width:12px;height:12px;display:inline-block;margin-right:5px;border:1px solid #ccc;"></span> < 8 </div>
          <div><span style="background:#ff8080;width:12px;height:12px;display:inline-block;margin-right:5px;"></span> ≥ 8 </div>`;
        ...
        const visibility = map.getLayoutProperty('indicateurs210dLayer', 'visibility');
        if (visibility === 'none') {
    ...
    // Ajout des couches vectorielles
      layersData.forEach(layer => {
        ...
        if (layer.type === 'fill') {
          if (layer.dynamicColor) {
            ...
            layerConfig.paint['fill-color'] = [
              ...
              ['<', ['get', layer.dynamicColor.property], 8], '#ffa6a6',
              ['>', ['get', layer.dynamicColor.property], 8], '#ff3636',
              '#d9d9d9' // fallback
            ];
          } else {
        ...
        map.addLayer(layerConfig);
         ...
         map.addLayer({
                  id: 'indicateurs210dLabels',
                  type: 'symbol',
                  source: 'indicateurs210dLayer', // le même id que ta source GeoJSON
                  layout: {
                    'text-field': ['get', 'nom'],
                    'text-size': 11,
        ...
        map.addLayer({
                 id: 'indicateurs210d-hover',
                 type: 'line',
                 source: 'indicateurs210dLayer', // même source
                 paint: {
                   'line-color': '#000',
                   'line-width': 2
                  },
      ...
      map.on('click', 'indicateurs210dLayer', (e) => {
        ...
        const s210dValue = parseFloat(feature.properties.s210d);
        const dommages = (Number(s210dValue) || 0).toLocaleString('fr-FR');
        ...
        <style>
          ...
          <center>${nom}</center>
            ...
            <p style="margin:auto;"><strong>Linéaire de réseaux de bus urbain <br> et de bus de transport départemental <br> impactés par la zone inondable 
 :</strong> ${dommages} km</p>
          </div>
        `;
         ...
         document.getElementById('donnees').innerHTML = `
          <div style="text-align:center; font-family:sans-serif; font-size:16px;">
            ...
            <div style="margin-top:10px;">
              <strong style="font-size:1.1em;"><i class="fas fa-bus"></i> Linéaire de réseaux de bus urbain et de bus de transport départemental impactés par la zone inondable :</strong> ${dommages} km
            </div>
          </div>
        `;
      });
      ...
      map.on('mousemove', 'indicateurs210dLayer', (e) => {
        if (e.features.length > 0) {
          const nomCommune = e.features[0].properties.nom;
      
          map.setFilter('indicateurs210d-hover', ['==', 'nom', nomCommune]);
      
          map.getCanvas().style.cursor = 'pointer';
        }
      });

        map.on('mouseleave', 'indicateurs210dLayer', () => {
          map.setFilter('indicateurs210d-hover', ['==', 'nom', '']);
    ...
    const aliasLayerIds = {
      ...
      indicateurs210dLayer: "Linéaire de réseaux de bus urbain et de bus de transport départemental impactés par la zone inondable",
...
indicateur_s210d <- st_read("layers/indicateurs.geojson", quiet = TRUE) %>%
  select(insee_com, nom, population, s210d) %>%
  mutate(
    s210d = replace_na(s210d, 0)
  ) %>%
  st_drop_geometry() %>%
  rename(
    Commune = nom,
    ...
    "Linéaire de réseaux de bus urbain et de bus de transport départemental impactés par la zone inondable" = s210d
  ) %>%
  mutate(
...
// Charger les données
data = FileAttachment("layers/data_s210d.json").json()

// Label du slider
html`<label for="slider-s210d" style="display: block;
  margin-top: 1em;
  font-weight: bold;
  margin-bottom: 1em;
...
data_s210d = FileAttachment("layers/data_s210d.json").json()

viewof slider_s210d = InputsTaux.range([0, 24], {
  value: 12,
  step: 4//
  //label: "Linéaire d'itinéraires routiers interceptés en km"
})

sorted_s210d = data_s210d
  .map(d => ({ ...d, s210d: +d.s210d || 0 }))
  .filter(d => d.s210d <= slider_s210d)
  .sort((a, b) => a.s210d - b.s210d)

PlotTaux.plot({
  marks: [
    PlotTaux.barY(sorted_s210d, {
      x: d => d.nom,
      y: d => d.s210d,
        fill: d => {
        const val = d.s210d;
        if (val === 0) return "#d9d9d9";
        if (val < 8) return "#ffa6a6";
        if (val >=8) return "#ff3636";
        return "#67000d"; // >4
        
      }
    }),
    PlotTaux.text(sorted_s210d, {
      x: d => d.nom,
      y: d => d.s210d,
      text: d => d.s210d.toLocaleString("fr-FR"),
      dy: -8,
      fontSize: 9,
      fill: "black",
      textAnchor: "middle"
  ...
  y: {
    grid: true,
    label: "Linéaire de réseaux de bus urbain et de bus de transport départemental impactés par la zone inondable",
    domain: [0, Math.max(...sorted_s210d.map(d => d.s210d))],
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    ...
    <th style="border:1px solid;">Ind2/10d</th>
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">Linéaire de réseaux de bus urbain et de bus de transport départemental impactés par la zone inondable</td>
  </tr>
  <tr>
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">Calcul du linéaire de voies de bus impacté :
      Celui-ci est calculé à partir des lignes de bus extraites depuis Open Street Map (OSM).
      Cette extraction s’obtient en filtrant sur le champ « other_tags » contenant la valeur
      « route »=>« bus » à partir de la couche multilinestrings. On intersecte ensuite ces deux tables
      avec la zone inondable (Zx) pour sélectionner les lignes inondés.
      Le résultat de l’indicateur est obtenu en agrégeant le linéaire des lignes de bus impactées par
      commune.</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">Les lignes de bus extraites de la base OSM n’ont pas de données d’altitude. Certaines lignes
        peuvent couper la zone inondable, et être malgré tout hors d’eau (pont, route en hauteur, etc.).
        La couche brute OSM contient dans un seul fichier, différents types de géométrie. Les lignes de
        bus sont extraites de l’objet « multilinestrings » et non de l’objet « Lines ».</td>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#BEDBE8; border:1px solid; font-weight: bold;">Table cible</td>
    <td colspan="3" style="border:1px solid;">p_obj2_cout_dommages.s210d_lignes_bus_zi</td>
  </tr>
  <tr>
  <tr>
    <td colspan="2" style="background-color:#BEDBE8; border:1px solid; font-weight: bold;">Variables mobilisées</td>
    <td colspan="3" style="border:1px solid;">Zx : zone inondable, <br>Res2 : voies de transport</td>
  </tr>
  <tr>
  <tr>
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    <td colspan="2" style="border:1px solid;">Couche multithématique OSM sur périmètre d’étude</td>
    <td style="border:1px solid;">2021</td>
    <td style="border:1px solid;">Locale</td>
    <td style="border:1px solid;">Shape</td>
    <td style="border:1px solid;">Polygone</td>
    <td colspan="2" style="border:1px solid;">Open Street Map (OSM)</td>
    <td colspan="2" style="border:1px solid;">osm_bus_ligne.shp</td>
  </tr>
  <tr>
