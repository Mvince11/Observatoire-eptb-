...
```{=html}
<script>
// Supprimer complètement le tabset Quarto
const tabset = document.querySelector(".tabset");
if (tabset) {
  // Récupérer le contenu de la carte
  const tab1 = tabset.querySelector("#tab1");

  if (tab1) {
    const carte = tab1.cloneNode(true); // clone propre

    // Supprimer le tabset entier
    tabset.remove();

    // Créer un conteneur plein écran
    const full = document.createElement("div");
    full.id = "fullmap-container";
    ...
    // Ajouter la carte dedans
    full.appendChild(carte);
