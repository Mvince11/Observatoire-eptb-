...
<div style="text-align:inherit !important; font-size:1.1em; font-family: calibri;margin-bottom:10%;"> Un établissement sensible est un établissement dans lequel l’évacuation de la population peut poser des difficultés, en raison de sa mobilité, de son état de santé ou de son nombre. Ces personnes courent des risques a priori analogues à ceux identifiés par ailleurs, mais leur manque d’autonomie les rend plus vulnérables et justifie leur identification.</div>
...
write_json(indicateurs, "layers/data_s14.json", auto_unbox = TRUE)
```
...
map = {
  ...
  maxzoom: 19
    ...
    // Créer le conteneur
        ...
        legend.innerHTML = `
          <strong>Nombre d'établissements <br> sensibles</strong><br>
          <div><span style="background:#d9d9d9;width:12px;height:12px;display:inline-block;margin-right:5px;"></span> 0</div>
          <div><span style="background:#ffffff;width:12px;height:12px;display:inline-block;margin-right:5px;border:1px solid #ccc;"></span> 1</div>
          <div><span style="background:#c359aa;width:12px;height:12px;display:inline-block;margin-right:5px;"></span> ≥ 2</div>
        `;
        ...
        const visibility = map.getLayoutProperty('indicateurs14Layer', 'visibility');
        if (visibility === 'none') {
    ...
    // Ajout des couches vectorielles
      layersData.forEach(layer => {
        ...
        if (layer.type === 'fill') {
          if (layer.dynamicColor) {
            ...
            layerConfig.paint['fill-color'] = [
              ...
              ['<=', ['get', layer.dynamicColor.property], 1], '#ffffff',
        ...
        map.addLayer(layerConfig);
         ...
         map.addLayer({
                  id: 'indicateurs14Labels',
                  type: 'symbol',
                  source: 'indicateurs14Layer', // le même id que ta source GeoJSON
                  layout: {
                    'text-field': ['get', 'nom'],
                    'text-size': 11,
        ...
        map.addLayer({
                 id: 'indicateurs14-hover',
                 type: 'line',
                 source: 'indicateurs14Layer', // même source
                 paint: {
                   'line-color': '#000',
                   'line-width': 2
                  },
      ...
      map.on('click', 'indicateurs14Layer', (e) => {
        ...
        const s14Value = parseFloat(feature.properties.s14);
        const dommages = !isNaN(s14Value)
          ? s14Value.toLocaleString('fr-FR')
          : 'Non disponible';
        ...
        <style>
          ...
          <center>${nom}</center>
            ...
            <p style="margin:auto;"><strong>Nombre d'établissements sensibles <br> en zone inondable :</strong> ${dommages}</p>
          </div>
        `;
        ...
        new maplibregl.Popup({closeButton: false})
         ...
         <div style="text-align:center; font-family:sans-serif; font-size:16px;">
            ...
            <div style="margin-top:10px;">
              <strong style="font-size:1.1em;"><i class="bi bi-building"></i> Nombre d'établissements sensibles en zone inondable :</strong> ${dommages}
            </div>
          </div>
      ...
      map.on('mousemove', 'indicateurs14Layer', (e) => {
        if (e.features.length > 0) {
          const nomCommune = e.features[0].properties.nom;
      
          map.setFilter('indicateurs14-hover', ['==', 'nom', nomCommune]);
      
          map.getCanvas().style.cursor = 'pointer';
        }
      });

        map.on('mouseleave', 'indicateurs14Layer', () => {
          map.setFilter('indicateurs14-hover', ['==', 'nom', '']);
    ...
    const aliasLayerIds = {
      ...
      indicateurs14Layer: 'Entreprises sensibles en ZI (nb)',
...
indicateur_s14 <- st_read("layers/indicateurs.geojson", quiet = TRUE) %>%
  select(insee_com, nom, population, s14) %>%
  mutate(
    s14 = replace_na(s14, 0)
  ) %>%
  st_drop_geometry() %>%
  rename(
    Commune = nom,
    ...
    `Nombre d'établissements sensibles en zone inondable` = s14
  ) %>%
  mutate(
    Population = format(Population, big.mark = " ", scientific = FALSE),
    `Nombre d'établissements sensibles en zone inondable` = format(`Nombre d'établissements sensibles en zone inondable`, big.mark = " ", scientific = FALSE)
  )

# Affichage avec reactable
reactable(indicateur_s14,
          filterable = TRUE,
          striped = TRUE,
          sortable = TRUE,
          resizable = TRUE,
...
// Charger les données
data = FileAttachment("layers/data_s14.json").json()

// Label du slider
html`<label for="slider-s14" style="display: block;
  margin-top: 1em;
  font-weight: bold;
  margin-bottom: 1em;
  ...
  margin-right: 20%;
  margin-left: 20%;">
  Etablissements sensibles en ZI (nb)
</label>`

// Slider


data_s14 = FileAttachment("layers/data_s14.json").json()

viewof slider_s14 = InputsTaux.range([0, 5], {
  value: 5,
  step: 1//,
  //label: "Etablissements sensibles en ZI (nb)"
})

sorted_s14 = data_s14
  .map(d => ({ ...d, s14: +d.s14 || 0 }))
  .filter(d => d.s14 <= slider_s14)
  .sort((a, b) => a.s14 - b.s14)

PlotTaux.plot({
  marks: [
    PlotTaux.barY(sorted_s14, {
      x: d => d.nom,
      y: d => d.s14,
        fill: d => {
        const val = d.s14;
        if (val === 0) return "#cdcdcd";
        if (val > 0 && val <= 1) return "#fff5f0";
        if (val > 1 && val <= 2) return "#fca487";
        if (val > 2 && val <= 4) return "#c359aa";
        return "#67000d"; // >4
      }
    }),
    PlotTaux.text(sorted_s14, {
      x: d => d.nom,
      y: d => d.s14,
      text: d => d.s14.toLocaleString("fr-FR"),
      dy: -8,
      fontSize: 9,
      fill: "black",
      textAnchor: "middle"
  ...
  y: {
    grid: true,
    label: "Etablissements sensibles en ZI (nb)",
    domain: [0, Math.max(...sorted_s14.map(d => d.s14))],
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    <th style="background-color:#EDB38C; border:1px solid;" colspan="2">Numéro d'indicateur</th>
    <th style="border:1px solid;">Ind1/4</th>
    <th style="background-color:#EDB38C; border:1px solid;">Date de mise à jour</th>
    <th style="border:1px solid;">31/05/2025</th>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#EDB38C; border:1px solid; font-weight: bold;">Intitulé</td>
    <td colspan="3" style="border:1px solid;">Nombre d’établissements sensibles (crèches, écoles, hôpitaux, maisons de retraite,
établissements pénitentiaires) en zone inondable</td>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#EDB38C; border:1px solid; font-weight: bold;">Objectif de la SNGRI</td>
    <td colspan="3" style="border:1px solid;">Objectif n°1 : Augmenter la sécurité des populations exposées</td>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#EDB38C; border:1px solid; font-weight: bold;">Axe de vulnérabilité</td>
    <td colspan="3" style="border:1px solid;">Axe 1/1 La mise en danger des personnes au sein des bâtiments</td>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#EDB38C; border:1px solid; font-weight: bold;">Source de vulnérabilité</td>
    <td colspan="3" style="border:1px solid;">S1/4 Vulnérabilité des publics de certains établissements dit sensibles</td>
  </tr>
</table>
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    <td colspan="2" style="background-color:#EDB38C; border:1px solid; font-weight: bold;">Modalités du calcul</td>
    <td colspan="3" style="border:1px solid;">A partir de la table établissements sensibles FINESS de l’ARS ('Centres de Santé','Centres
    Hospitaliers','Centres Hospitaliers Régionaux','Centres Hospitaliers Spécialisés Lutte Maladies
    Mentales','Dialyse Ambulatoire','Etab. et Services d'Hébergement pour Adultes
    Handicapés','Etablissements de Soins de Courte Durée','Etablissements de Soins de Longue
    Durée','Etablissements d'Hébergement pour Personnes Âgées','Etablissements et Services
    Hébergement Enfants Handicapés','Transfusion Sanguine'), des couches « campings » et
    « écoles » (extraite du fichier annuaire_education, où on ne retient que les écoles
    maternelles et primaires), on sélectionne les entités qui intersectent la zone inondable (buffer
    de 50m autour des points pour simuler l’emprise du bâtiment.</td>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#EDB38C; border:1px solid; font-weight: bold;">Échelle de représentation</td>
    <td colspan="3" style="border:1px solid;">Communale</td>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#EDB38C; border:1px solid; font-weight: bold;">Points de vigilance</td>
    <td colspan="3" style="border:1px solid;">RAS</td>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#EDB38C; border:1px solid; font-weight: bold;">Table cible</td>
    <td colspan="3" style="border:1px solid;">p_obj1_securite_personnes.s14_pop_sens_zi</td>
  </tr>
  <tr>
  <tr>
    <td colspan="2" style="background-color:#EDB38C; border:1px solid; font-weight: bold;">Variables mobilisées</td>
    <td colspan="3" style="border:1px solid;">Zx : zone inondable,
Pop3 : Population sensible</td>
  </tr>
  <tr>
  <tr>
    <td colspan="2" style="background-color:#EDB38C; border:1px solid; font-weight: bold;">Indicateur commun SLGRI Vienne/Clain</td>
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    <td colspan="10" style="background-color:#EDB38C; border:1px solid; font-weight: bold;">Données sources</td>
  </tr>
  <tr>
    <th style="background-color:#EDB38C; border:1px solid;" colspan="2">Désignation</th>
    <th style="background-color:#EDB38C; border:1px solid;">Millésime</th>
    <th style="background-color:#EDB38C; border:1px solid;">Nationale/locale</th>
    <th style="background-color:#EDB38C; border:1px solid;">Format</th>
    <th style="background-color:#EDB38C; border:1px solid;">Type</th>
    <th style="background-color:#EDB38C; border:1px solid;" colspan="2">Producteur</th>
    <th style="background-color:#EDB38C; border:1px solid;" colspan="2">Nom fichier</th>
  </tr>
  <tr>
  ...
  <tr>
    <td colspan="2" style="border:1px solid;">Annuaire éducation</td>
    <td style="border:1px solid;">2021</td>
    <td style="border:1px solid;">Nationale</td>
    <td style="border:1px solid;">Shape</td>
    <td style="border:1px solid;">Point</td>
    <td colspan="2" style="border:1px solid;">Education nationale</td>
    <td colspan="2" style="border:1px solid;">annuaire_education.shp</td>
  </tr>
  <tr>
    <td colspan="2" style="border:1px solid;">Établissements sensibles</td>
    <td style="border:1px solid;">2019</td>
    <td style="border:1px solid;">Nationale</td>
    <td style="border:1px solid;">Shape</td>
    <td style="border:1px solid;">Point</td>
    <td colspan="2" style="border:1px solid;">ARS</td>
    <td colspan="2" style="border:1px solid;">finess_pt_etab_juin2019_d86.shp</td>
  </tr>
  <tr>
