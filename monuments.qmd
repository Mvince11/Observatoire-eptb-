...
<div style="text-align:inherit !important; font-size:1.1em; font-family: calibri;margin-bottom:10%;"> Les monuments historiques, bâtiments patrimoniaux et musées, ainsi que les collections qu’ils abritent sont susceptibles d’être endommagés par une inondation. Il s’agit d’identifier les principaux bâtiments exposés. Seul un diagnostic individuel peut permettre d’aller plus loin et d’identifier la vulnérabilité de chaque enjeu.</div>
...
indicateurs <- st_read("layers/indicateurs.geojson", quiet = TRUE) %>%
  st_drop_geometry() %>%
  select(nom, s24) %>%
   mutate(s24 = replace_na(s24, 0))

# Sauvegarder au format JSON dans le dossier _site
write_json(indicateurs, "layers/data_s24.json", auto_unbox = TRUE)
...
map = {
  ...
  maxzoom: 19
    ...
    // Créer le conteneur
        ...
        legend.innerHTML = `
          <strong>Monuments en zone inondable</strong><br>
          ...
          <div><span style="background:#00441b;width:12px;height:12px;display:inline-block;margin-right:5px;border:1px solid #ccc;"></span> ≥ 2 </div>
        `;
        ...
        const visibility = map.getLayoutProperty('indicateurs24Layer', 'visibility');
        if (visibility === 'none') {
    ...
    // Ajout des couches vectorielles
      layersData.forEach(layer => {
        ...
        if (layer.type === 'fill') {
          if (layer.dynamicColor) {
            ...
            layerConfig.paint['fill-color'] = [
              ...
              ['>', ['get', layer.dynamicColor.property], 2], '#00441b',
              '#d9d9d9' // fallback
            ];
          } else {
        ...
        map.addLayer(layerConfig);
         ...
         map.addLayer({
                  id: 'indicateurs24Labels',
                  type: 'symbol',
                  source: 'indicateurs24Layer', // le même id que ta source GeoJSON
                  layout: {
                    'text-field': ['get', 'nom'],
                    'text-size': 11,
        ...
        map.addLayer({
                 id: 'indicateurs24-hover',
                 type: 'line',
                 source: 'indicateurs24Layer', // même source
                 paint: {
                   'line-color': '#000',
                   'line-width': 2
                  },
      ...
      map.on('click', 'indicateurs24Layer', (e) => {
        ...
        const s24Value = parseFloat(feature.properties.s24);
        const dommages = (Number(s24Value) || 0).toLocaleString('fr-FR');
        ...
        <style>
          ...
          <center>${nom}</center>
            ...
            <p style="margin:auto;"><strong>Monuments en zone inondable :</strong> ${dommages}</p>
          </div>
        `;
        ...
        new maplibregl.Popup({closeButton: false})
         ...
         <strong style="font-size:1.1em"><i class="fas fa-landmark"></i> Monuments en zone inondable :</strong> ${dommages}<br>
         `;
      });



      map.on('mousemove', 'indicateurs24Layer', (e) => {
        if (e.features.length > 0) {
          const nomCommune = e.features[0].properties.nom;
      
          map.setFilter('indicateurs24-hover', ['==', 'nom', nomCommune]);
      
          map.getCanvas().style.cursor = 'pointer';
        }
      });

        map.on('mouseleave', 'indicateurs24Layer', () => {
          map.setFilter('indicateurs24-hover', ['==', 'nom', '']);
    ...
    const aliasLayerIds = {
      ...
      indicateurs24Layer: "Monuments en zone inondable",
...
indicateur_s24 <- st_read("layers/indicateurs.geojson", quiet = TRUE) %>%
  select(insee_com, nom, population, s24) %>%
  mutate(
    s24 = replace_na(s24, 0)
  ) %>%
  st_drop_geometry() %>%
  rename(
    Commune = nom,
    ...
    `Monuments en zone inondable` = s24
  ) %>%
  mutate(
...
// Charger les données
data = FileAttachment("layers/data_s24.json").json()
...
html`<label for="slider-s114c" style="display: block;
  ...
  margin-right: 20%;
  margin-left: 20%;">
  Monuments en zone inondable
</label>`

// Slider


data_s24 = FileAttachment("layers/data_s24.json").json()

viewof slider_s24 = InputsTaux.range([0, 9], {
  value: 5,
  step: 3//,
...
sorted_s24 = data_s24
  .map(d => ({ ...d, s24: +d.s24 || 0 }))
  .filter(d => d.s24 <= slider_s24)
  .sort((a, b) => a.s24 - b.s24)

PlotTaux.plot({
  marks: [
    PlotTaux.barY(sorted_s24, {
      x: d => d.nom,
      y: d => d.s24,
        fill: d => {
        const val = d.s24;
        ...
        if (val > 2) return "#00441b";
        return "#7bc87c"; // >4
      }
    }),
    PlotTaux.text(sorted_s24, {
      x: d => d.nom,
      y: d => d.s24,
      text: d => d.s24.toLocaleString("fr-FR"),
      dy: -8,
      fontSize: 9,
      fill: "black",
      textAnchor: "middle"
  ...
  y: {
    grid: true,
    label: "Monuments en zone inondable",
    domain: [0, Math.max(...sorted_s24.map(d => d.s24))],
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    ...
    <th style="border:1px solid;">Ind2/4a – 2/4b</th>
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">Nombre de musées, bâtiments patrimoniaux et monuments historiques susceptibles d’être endommagés</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">S2/4 Dommage divers au patrimoine culturel</td>
  </tr>
</table>
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">Calcul du nombre de bâtiments patrimoniaux impactés :
L’objectif est alors de croiser les entités de points correspondant aux bâtiments patrimoniaux
avec la zone inondable (Zx). Le résultat de l’indicateur est l’agrégation par commune du nombre
de bâtiments situés en zone inondable.</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td colspan="3" style="border:1px solid;">Les couches utilisées sont des couches nationales (BDTOPO de l’IGN et BD TIGRE du ministère de la Culture), qui peuvent être incomplètes. L’usage d’une couche locale plus précise peut
améliorer la qualité de l’indicateur.</td>
  </tr>
  <tr>
    <td colspan="2" style="background-color:#88A9E3; border:1px solid; font-weight: bold;">Table cible</td>
    <td colspan="3" style="border:1px solid;">p_obj2_cout_dommages.s24_mh_patrim_zi</td>
  </tr>
  <tr>
  <tr>
    <td colspan="2" style="background-color:#88A9E3; border:1px solid; font-weight: bold;">Variables mobilisées</td>
    <td colspan="3" style="border:1px solid;">Zx : zone inondable,<br> Oc9 : musées et bâtiments patrimoniaux, <br> Oc10 : monuments, édifices patrimoniaux, remarquables, historiques</td>
  </tr>
  <tr>
  <tr>
...
<table style="width:90%; border-collapse: collapse; border:1px solid; text-align:center; margin:auto;">
  ...
  <tr>
    <td colspan="2" style="border:1px solid;">Zone d’activité ou d’intérêt BDTOPO</td>
    <td style="border:1px solid;">2020</td>
    <td style="border:1px solid;">Nationale</td>
    <td style="border:1px solid;">Shape</td>
    <td style="border:1px solid;">Polygone</td>
  ...
  <tr>
    <td colspan="2" style="border:1px solid;">BD TIGRE</td>
    <td style="border:1px solid;">2021</td>
    <td style="border:1px solid;">Nationale</td>
    <td style="border:1px solid;">Shape</td>
    <td style="border:1px solid;">Polygone</td>
    <td colspan="2" style="border:1px solid;">CULTURE</td>
    <td colspan="2" style="border:1px solid;">imm86_exporttigrepolygone.shp</td>
  </tr>
  <tr>
